...
func postHandler(e *Envelope, c *Connection) {
	...
	for kind, data := range e.Data {
		...
		if err != nil {
			PrimaryHub.SendJSON(c.Agent, util.ApiError{401, 400, "Unrecognized type", ""})
			return
		}

		if err := models.PopulateModel(model, &data); err != nil {
			PrimaryHub.SendJSON(c.Agent, util.ApiError{400, 400, "Error populating model with json data", "I need to check maself"})
			return
		}
